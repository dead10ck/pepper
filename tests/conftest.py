...
def pepper_cli(request, session_salt_api, salt_api_port, session_sshd_server):
    """
...
def session_minion_factory(session_master_factory):
    ...
    minion_factory = session_master_factory.salt_minion_daemon(
...
@pytest.fixture(scope="session")
def session_pepper_no_login(session_master, session_salt_api_factory):
    with (
        patch(
            "pepper.cli.PepperCli.login",
            MagicMock(side_effect=lambda api, login_details: None),
        ),
        patch("pepper.cli.getpass.getpass", MagicMock(return_value=None)),
        patch("pepper.cli.input", MagicMock(return_value=None)),
    ):
        import pepper
        yield pepper
